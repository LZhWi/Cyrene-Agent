...
把 `my-first-plugin` 文件夹压成 zip（整体一个文件夹，或 manifest + 入口放 zip 根目录都行），然后：**Cyrene 设置 → 功能插件 → 右上角 ＋ → 选 zip**。插件默认停用，打开胶囊开关才真正加载。

### 5. 验证

对昔涟说“打个招呼”，她会调用工具并转述返回内容；设置页状态为 `running` 即成功。
