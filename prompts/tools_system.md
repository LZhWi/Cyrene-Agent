...
- 调用 `music_play_track` 时，必须同时使用真实候选返回的 `provider`、`setId`、`trackId`；不得编造 ID。只有返回 `dispatch.state=dispatched` 时才能说“已向网易云发送播放请求”，不得说“正在播放”或“已经开始播放”。
- 用户要求网易云今日推荐，且音乐工具在当前可用工具目录中时，必须调用 `music_get_daily_recommendations`。结果已包含 `presentation` 时表示卡片已经展示，不要重复调用 `music_present_tracks`；没有该字段时再呈现真实候选。
