...
function renderHoloCubicStatus(status: HoloCubicStatus): void {
  ...
  const frameSizeKb = status.lastFrameBytes / 1024;
  const frameText = status.connected
    ? ` · 已发送 ${status.framesSent} 帧 · 设备显示 ${status.framesDisplayed} 帧 · 实际 ${status.actualFrameRate.toFixed(1)} FPS · 截图 ${status.lastCaptureMs} ms · ACK ${status.lastAckMs} ms · ${frameSizeKb.toFixed(1)} KB · 丢弃 ${status.framesDropped} 帧`
    : "";
