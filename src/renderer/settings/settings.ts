...
if (!window.settings) {
  (window as unknown as { settings: SettingsApi }).settings = {
    ...
    getHoloCubicStatus: () => Promise.resolve({ state: "stopped", connected: false, framesCaptured: 0, framesSent: 0, framesDisplayed: 0, framesDropped: 0, reconnectAttempt: 0, bufferedBytes: 0, lastFrameAt: null, lastError: "", inputEvents: 0, lastInput: null }),
...
function renderHoloCubicStatus(status: HoloCubicStatus): void {
  ...
  const frameText = status.connected ? ` · 已发送 ${status.framesSent} 帧 · 设备显示 ${status.framesDisplayed} 帧 · 丢弃 ${status.framesDropped} 帧` : "";
...
// Gamebot 的插件页只负责启用状态；完整配置与运行界面位于独立窗口。
function initGameBotPluginToggle(): void {
  const enabled = document.getElementById("plugin-gamebot-enabled") as HTMLInputElement | null;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const gameBot = (window as any).gameBot as {
    getConfig: () => Promise<{ enabled: boolean }>;
    saveConfig: (config: { enabled: boolean }) => Promise<unknown>;
  } | undefined;
  if (!enabled || !gameBot) return;
  void gameBot.getConfig().then((config) => { enabled.checked = config.enabled; });
  enabled.addEventListener("change", () => {
    void gameBot.saveConfig({ enabled: enabled.checked });
  });
}
...
const PERMISSION_NOTES: Record<PermissionLevel, string> = {
  ...
  "scoped": "指定目录（兼容）：允许文件读写和网络访问，禁止直接命令执行；目录限制将在后续重新设计。",
...
function paintPermissionUI(level: PermissionLevel): void {
  ...
  // scoped 档已从插件面板移除，旧值按兼容行为回退显示只读
