...
describe("MusicMcpClient", () => {
  ...
  it("verifyContractOnConnect ok when all tools present", async () => {
    listTools.mockResolvedValue({ tools: [
      ...
      tool("cloud_music_my_playlists", {}),
      tool("cloud_music_playlist_detail", { playlist_id: { type: "string" } }),
      tool("cloud_music_create_playlist", { name: { type: "string" } }),
      tool("cloud_music_add_to_playlist", { playlist_id: { type: "string" }, track_ids: { type: "array" } }),
      tool("cloud_music_my_subscriptions", { category: { type: "string" } }),
