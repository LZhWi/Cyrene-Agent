...
import { createSemanticDocumentChunks, type SemanticChunkingOptions } from "./semantic-chunk";
...
/** 纯隔离评测：不解析 userData 路径，也不写文件或 RAG 状态。 */
export async function evaluateDocumentChunkingSandbox(input: {
  ...
  const semantic = await createSemanticDocumentChunks(
    ...
    async (texts) => { embeddingBatchCalls += 1; return input.embedBatch(texts); },
    input.options,
  );
