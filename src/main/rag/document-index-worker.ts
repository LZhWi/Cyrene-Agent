      ...
      messageChain = messageChain.then(async () => {
        ...
        if (message.type === "prepared") {
          ...
          if (cached) {
            ...
            finish({ kind: "indexed", name: prepared.name, chunks: cached.chunkCount, importId: cached.importId, cached: true, text: prepared.inlineText });
            return;
          }
        ...
        if (message.type === "completed") {
          ...
          finish({ kind: "indexed", name: prepared.name, chunks: persistedChunks, importId, text: prepared.inlineText });
        }
      }).catch((error) => finish(indexingFailureResult(job.input.filePath, prepared, error instanceof Error ? error.message : String(error))));
    });
    worker.on("error", (error: Error) => finish(indexingFailureResult(job.input.filePath, prepared, error.message)));
    worker.on("exit", (code: number) => {
      if (!settled && code !== 0) finish(indexingFailureResult(job.input.filePath, prepared, `document worker exited with code ${code}`));
    });
...
function createDefaultRunnerDependencies(): DocumentIndexWorkerRunnerDependencies {
  return {
    ...
    getCachedImport: async (textSha256) => {
      ...
      const current = await cache.getValidDocumentCacheRecord(cache.createDocumentCacheKey(identity), rag.hasImportedDocumentChunks);
      if (current) return current;
      const legacyIdentity = await cache.buildDocumentCacheIdentityFromTextSha(textSha256, cache.LEGACY_DOCUMENT_CHUNK_STRATEGY_VERSION);
      return cache.getValidDocumentCacheRecord(cache.createDocumentCacheKey(legacyIdentity), rag.hasImportedDocumentChunks);
    },
    getEmbeddingConfig: () => {
      ...
      try {
        semantic = await createSemanticDocumentChunks(
          ...
          (texts) => provider.embedBatch(texts),
          {
            ...
            onProgress: ({ completed, total }) => port.postMessage({ type: "progress", completedChunks: completed, totalChunks: total } satisfies WorkerOutboundMessage),
          },
        );
      } catch (error) {
