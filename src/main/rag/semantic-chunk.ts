...
export interface SemanticPreparedChunk extends Chunk { embedding: number[] }
...
export interface SemanticChunkingOptions {
  ...
  isCancelled?: () => boolean;
  onProgress?: (progress: { phase: "semantic-boundaries" | "chunk-embeddings"; completed: number; total: number }) => void;
}
...
function rawDocumentSpans(text: string, atomicTarget: number): RawSpan[] {
  ...
  for (const rawLine of lines) {
    ...
    if (inFence) {
      fenceText += rawLine;
      if (isFence) {
        spans.push(...splitOversizedSpan({ text: fenceText.trim(), start: fenceStart, end: offset + rawLine.length, hardBoundaryBefore: true, headingPath: fenceHeadingPath }, atomicTarget));
        inFence = false;
        fenceText = "";
      }
    ...
    if (!trimmed) { offset += rawLine.length; continue; }
    ...
    if (headingMatch) {
      ...
      spans.push({ text: trimmed, start, end: start + trimmed.length, hardBoundaryBefore: true, headingPath: headings.map((heading) => heading.title) });
    } else {
      const headingPath = headings.map((heading) => heading.title);
      for (const span of splitLineAtSentences(line, offset, headingPath)) spans.push(...splitOversizedSpan(span, atomicTarget));
    }
    offset += rawLine.length;
  }
  if (inFence && fenceText.trim()) {
    spans.push(...splitOversizedSpan({ text: fenceText.trim(), start: fenceStart, end: text.length, hardBoundaryBefore: true, headingPath: fenceHeadingPath }, atomicTarget));
  }
  return spans;
}

export function createSemanticDocumentUnits(text: string, atomicTarget = SEMANTIC_ATOMIC_TARGET): SemanticDocumentUnit[] {
  ...
  const flush = () => {
    ...
    units.push({ text: unitText, start, end, tokens: estimateTokens(unitText), hardBoundaryBefore: pending[0].hardBoundaryBefore, headingPath: [...pending[0].headingPath] });
    pending = [];
    pendingTokens = 0;
  };
...
function weightedCentroid(vectors: number[][], weights: number[]): number[] {
  ...
  return vectors[0].map((_, dimension) => vectors.reduce((sum, vector, index) => sum + vector[dimension] * weights[index], 0) / totalWeight);
}

async function embedInBatches(texts: string[], embedBatch: (texts: string[]) => Promise<number[][]>, batchSize: number, phase: "semantic-boundaries" | "chunk-embeddings", options: SemanticChunkingOptions): Promise<number[][]> {
...
export async function createSemanticDocumentChunks(text: string, source: string, embedBatch: (texts: string[]) => Promise<number[][]>, options: SemanticChunkingOptions = {}): Promise<SemanticChunkingResult> {
  ...
  const units = createSemanticDocumentUnits(text, Math.min(options.atomicTarget ?? SEMANTIC_ATOMIC_TARGET, maxTokens));
  ...
  units.forEach((unit, index) => {
    if (currentUnits.length === 0) {
      currentUnits = [unit]; currentVectors = [unitVectors[index]]; currentTokens = unit.tokens; return;
    }
    ...
    if (!exceedsMax && !crossesStructure && (currentTokens < minTokens || semanticallyClose)) {
      currentUnits.push(unit); currentVectors.push(unitVectors[index]); currentTokens += unit.tokens;
    } else {
      flush(); currentUnits = [unit]; currentVectors = [unitVectors[index]]; currentTokens = unit.tokens;
    }
  });
  flush();
  ...
  return {
    chunks: chunkTexts.map((chunkText, index) => ({ id: `${source}_${index}`, text: chunkText, source, index, embedding: chunkVectors[index] })),
    atomicUnitCount: units.length,
    similarityComparisons: comparisons,
    embeddingTexts: units.length + chunkTexts.length,
    estimatedEmbeddingTokens: units.reduce((sum, unit) => sum + unit.tokens, 0) + chunkTexts.reduce((sum, chunkText) => sum + estimateTokens(chunkText), 0),
  };
}
