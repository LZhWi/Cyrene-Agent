...
- `icon`（可选）：插件图标文件名，支持 `.png` / `.jpg` / `.webp` / `.svg`（≤2MiB），会在设置页插件卡片左侧展示；不写不影响任何功能，写了但文件有问题会被静默忽略（插件照常加载）
...
然后：**Cyrene 设置 → 功能插件 → 右上角 ＋ → 选 zip**。插件出现在列表且默认停用——这是刻意的安全设计，装进来不等于运行，打开胶囊开关才真正加载。

### 5. 验证

对昔涟说“打个招呼”，她会调用你注册的工具并转述返回内容。去设置页看插件状态是 `running` 就说明成功。
...
实现 `open()` 后，设置页的插件卡片会多一个“打开”按钮。最省事的做法：`ui.html` 随插件打包，用 `BrowserWindow` 加载。
