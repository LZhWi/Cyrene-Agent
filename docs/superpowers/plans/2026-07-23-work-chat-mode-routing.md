# Work and Chat Mode Routing Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make tool permission `off` and desktop Chat mode run the Chat pipeline, while `safe-only` and `all` retain the complete Work pipeline.

**Architecture:** Add an explicit `executionMode` to the AG-UI input and Cyrene runtime options. `buildAgentRunOptions` resolves the mode before CITA preparation; `CyreneAgent` dispatches Chat runs to a dedicated one-call loop and Work runs to the existing LangGraph/legacy runtime. Channel settings keep their existing sandbox field and add an `off` value.

**Tech Stack:** TypeScript, Electron, AG-UI, RxJS, Vitest.

## Global Constraints

- Chat must not invoke full CITA, Action Gate, Native Function Calling, permissions, or Tool Runtime.
- Chat must expose zero tools, including music tools.
- Chat must preserve the Soul system, conversation history, image fallback, usage accounting, AG-UI text events, and existing post-run side effects.
- Existing `safe-only` and `all` settings must retain current Work behavior.
...
### Task 2: Resolve Chat before full CITA and tools
...
**Interfaces:**
- Produces: `type AgentExecutionMode = "work" | "chat"`
- Consumes: `AguiRunInput.executionMode?: AgentExecutionMode`
- Produces: `CyreneRunOptions.executionMode: AgentExecutionMode`

- [ ] **Step 1: Replace the old mode-specific music test with failing Chat assertions**
...
Add a second test proving an explicit channel `executionMode: "chat"` is independent from style, and keep the existing CITA Work test.

- [ ] **Step 2: Run the tests and verify RED**

Run: `npm test -- src/main/orchestrator/build-options.test.ts`

Expected: FAIL because Chat still preserves music tools and calls CITA before resolving mode.
...
Guard `prepareCitaTurn` with `!isChatMode`, set `runTools` to `[]` for Chat, and include `executionMode` on `CyreneRunOptions`.
...
### Task 3: Add the dedicated Chat runtime

**Files:**
- Create: `src/main/orchestrator/chat-loop.ts`
- Create: `src/main/orchestrator/chat-loop.test.ts`
- Modify: `src/main/orchestrator/cyrene-agent.ts`
- Modify: `src/main/orchestrator/cyrene-agent-runtime.test.ts`

**Interfaces:**
- Produces: `runChatLoop(options: ChatLoopOptions): Promise<TwoPhaseFcResult>`
...
Run: `npm test -- src/main/orchestrator/chat-loop.test.ts`

Expected: FAIL because the module does not exist.

- [ ] **Step 3: Implement the minimal Chat loop**
...
In `runWithEvents`, call `runChatLoop` before constructing execution-ledger/tool callbacks when the mode is Chat. Otherwise retain the current runtime branch unchanged.

- [ ] **Step 5: Run runtime tests and verify GREEN**

Run:

`npm test -- src/main/orchestrator/chat-loop.test.ts src/main/orchestrator/cyrene-agent-runtime.test.ts`
...
```text
切换至 Chat，不启用 CITA、Action Gate 或任何工具。
```
...
- Test: `src/main/channels/settings-store.test.ts`
- Test: `src/main/orchestrator/build-options.test.ts`
- Test: `src/main/orchestrator/chat-loop.test.ts`
...
```powershell
npm test -- src/main/channels/settings-store.test.ts src/main/orchestrator/build-options.test.ts src/main/orchestrator/chat-loop.test.ts src/main/orchestrator/cyrene-agent-runtime.test.ts src/main/orchestrator/langgraph-agent-loop.test.ts
```
...
```powershell
git add -- src/main/channels/settings-store.ts src/main/channels/settings-store.test.ts src/main/agui-bridge.ts src/main/orchestrator/build-options.ts src/main/orchestrator/build-options.test.ts src/main/orchestrator/chat-loop.ts src/main/orchestrator/chat-loop.test.ts src/main/orchestrator/cyrene-agent.ts src/main/orchestrator/cyrene-agent-runtime.test.ts src/renderer/settings/index.html src/renderer/settings/settings.ts src/main/index.ts docs/superpowers/plans/2026-07-23-work-chat-mode-routing.md
