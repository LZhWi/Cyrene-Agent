...
describe("aging 压缩聚类只读候选", () => {
  it("梦境压缩按本地算法用种子相似边形成完整分组并返回最低分", () => {
    ...
    expect(findCompressionClusters(entries, pairs)).toEqual([{ entryIds: ["a", "b", "c", "d", "e", "f"], minimumScore: 0.83 }]);
  });
  ...
  it("常规压缩只选 active，使用 0.85 阈值并要求全组两两相似", () => {
    const entries = ["a", "b", "c", "d"].map((id) => make(id, { status: "active" }));
    const pairs = [
      { leftId: "a", rightId: "b", score: 0.9 },
      { leftId: "a", rightId: "c", score: 0.9 },
      { leftId: "b", rightId: "c", score: 0.86 },
      { leftId: "a", rightId: "d", score: 0.99 },
      { leftId: "b", rightId: "d", score: 0.84 },
    ];
    expect(findCompressionClusters(entries as any, pairs, { status: "active", minimumScore: 0.85 }))
      .toEqual([{ entryIds: ["a", "b", "c"], minimumScore: 0.9 }]);
  });
});
