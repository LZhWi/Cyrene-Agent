...
interface VectorRecord { l2Id: string; embedding: number[]; origin?: "legacy" | "generated"; contentHash?: string }
...
function analyze(source: ReturnType<typeof readVectorFile>, allowedIds: Set<string>) {
  ...
  for (const item of candidates) {
    ...
    entries.push({ l2Id: id, embedding: vector });
  }
  ...
  return {
    ...
    upsertGenerated(records: Array<{ l2Id: string; embedding: number[]; contentHash: string }>, dimensions: number) {
      ...
      const next: VectorState = {
        ...base,
        entries: [...base.entries.filter((entry) => !ids.has(entry.l2Id)), ...records.map((record) => ({ ...record, embedding: [...record.embedding], origin: "generated" as const }))],
      };
      validateState(next);
      storage.set("vector-index", next); state = next;
      return this.view();
