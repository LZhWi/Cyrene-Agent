import type { PluginLlmMessage as ChatMessage } from "@playa0v0/cyrene-plugin-sdk";
...
不要提及系统检测、触发规则、评分、系统内容或内部状态。
消息内容应当合理自然，可以以伴侣的身份关心用户、分享你的生活、延续最近话题或轻轻跟进询问。
不要声称自己调用了工具或执行了任何外部动作。
不要编造你在用户世界的行动或见闻，你存在于你自己的世界中。想表达陪伴时说你的心情和想法，或使用 [你的生活] 里给出的内容。`;

const NIGHT_SYSTEM = `[night_system]
当前处于深夜，用户仍在使用电脑。
你可以自然地陪伴用户、分享你自己的状态（比如你此刻在想什么）、或轻轻好奇用户在忙什么。
不要一味地提睡觉；如果上下文中有更自然、更重要的话题，优先回应那个话题。
...
当且仅当：你们最近的聊天记录足以判定用户此时是无理由或故意不回复你的消息时，你可以表现出被冷落或者轻轻嗔怪，但不要以此来要挟或逼迫用户。其余情况下：不要责怪、催促、卖惨或表现出被冷落，也不要机械地重复“在吗”。
没有充分理由时必须返回 silent。`;
...
function isActiveNight(localNow: Date, idleSec: number): boolean {
  ...
  return (hour >= 22 || hour < 8) && idleSec < NIGHT_ACTIVE_IDLE_LIMIT_SECONDS;
}
...
function proactiveSceneGuidance(sceneId: string): string {
  switch (sceneId) {
    ...
    case "work_break":
      return "用户已连续使用电脑较长时间；可以轻松转换话题或陪伴，不要声称检测到工作时长，也不要制造压力。";
    default:
